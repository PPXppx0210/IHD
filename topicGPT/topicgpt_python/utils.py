...
class APIClient:
    ...
    def __init__(self, api, model):
        ...
        # Setting API key ----
        if api == "openai":
            self.client = OpenAI(base_url="xxxxxxxx", api_key="xxxxxxxxxxxx")
    ...
    def iterative_prompt(
        ...
        for attempt in range(num_try):
            ...
            #         raise
                ...
                print(f"Attempt {attempt + 1}/{num_try} failed: {e}")
                if attempt == num_try - 1:
                    print(f"All attempts failed for this request. Continuing with next request.")
                time.sleep(5)  # Delay between retries to handle rate limiting or transient errors
