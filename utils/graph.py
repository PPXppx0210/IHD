...
def sample_tweets_around_user(users:set, ut_mp:dict, tweets_per_user:int, counts_matter:bool=False, return_edges:bool=False):
    """
    鍔熻兘: 鏍规嵁ut_mp涓簎sers涓殑姣忎釜user鎸戦€塵in(tweets_per_user, len(ut_mp[user]))涓帹鏂囬偦灞呰妭鐐?    鍙傛暟: 
        counts_matter琛ㄧず鎬诲叡鎸戦€夌殑鎺ㄦ枃鑺傜偣鏁伴噺蹇呴』绛変簬len(users)*tweets_per_user,涓嶆弧瓒冲垯杩斿洖绌烘帹鏂囪妭鐐归泦鍚?    杩斿洖:
...
def extend_wholegraph(g, ut_mp, initial_feats, tweet_per_user=20, sparse_graph=True):
    """
    鍔熻兘: 鍦╯ubg_deg483瀛愬浘鍜寀t_mp鐨勫熀纭€涓? 鏍规嵁tweet_per_user鍙傛暟閲嶆柊鐢熸垚hadjs鍜宖eats
    鍙傛暟: sparse_graph=False鏃? 杩斿洖鐨勫疄闄呬笂鏄竴鍚岃川鍥?    """
...
def search_cascades(user_texts_mp:Dict[int,dict], subg:igraph.Graph)->Dict[int,list]:
    """
    鍔熻兘: 灏嗙鍚堢浉鍚屾潯浠剁殑鎺ㄦ枃缁勭粐涓哄悓涓€璇濋绾ц仈, 杩欎簺鏉′欢鍖呮嫭,
        1. 鍖呭惈鐩稿悓tag, i.e. #blacklivesmatter
        2. 鍖呭惈鐩稿悓url, i.e. 
        3. 鐢?RT @USER"褰㈠紡鏍囪瘑鐨勫涓枃鏈唴瀹圭浉鍚岀殑鎺ㄦ枃
    """
    ...
    # NOTE: 'RT @USER: '杩欎竴鏂瑰紡寰楀埌鐨勭骇鑱斿熀鏈TAG绾ц仈鍜孶RL绾ц仈鎵€瑕嗙洊, 鍥犳涓嶅啀閲囩敤杩欑鏂瑰紡鏋勯€犵骇鑱?    # timelines_retweet = aggby_retweet_info(user_texts_mp, userid2graphid)
...
def aggby_same_text(user_texts_mp:Dict[int,dict], regex_pattern:str, userid2graphid:Dict[int,int])->Dict[int,list]:
    ...
    鍙傛暟: 
        ...
        regex_pattern: 杩囨护璇濋绾ц仈鐨勬鍒欒〃杈惧紡妯″紡
            i.e. tag_pattern = r'(#\w+)', url_pattern = r'(https?://[a-zA-Z0-9.?/&=:]+)'
        userid2graphid: 鍘熷鎷撴墤鍥?v=208894)涓庨噰鏍峰瓙鍥?v=44896)鐨勮妭鐐笽D瀵瑰簲鍏崇郴
    ...
    for user_id, raw_texts in user_texts_mp.items():
        ...
        for _, text in raw_texts.items():
            ...
            for match in group:
                ...
                if end_pos == len(text['text']) - 1 and not text['text'][-1].isalnum():
                    continue
...
def aggby_retweet_info(user_texts_mp:Dict[int,dict], userid2graphid:Dict[int,int]):
    """
    鎬濊矾: 灏嗗幓鎺?RT @USER: "褰㈠紡鍚? 鍖呭惈鐩稿悓鏂囨湰鍐呭鐨勬帹鏂囩粍缁囨垚绾ц仈
        1. 鍏堝皢鍖呭惈"RT @USER: "褰㈠紡鐨勬帹鏂囨壘鍑烘潵, 骞惰仛鍚堝舰鎴愮骇鑱?
        2. 鍐嶉拡瀵规瘡涓笂杩扮骇鑱? 鏆村姏鎼滅储鎵€鏈夋帹鏂? 鏌ユ壘鏄惁瀛樺湪杩欎簺杞彂鎺ㄦ枃鐨勫師濮嬫帹鏂? 濡傛灉瀛樺湪鍒欏姞鍏ヨ绾ц仈涓?
    """
    # Aggregate RT Info
...
def aggregate_texts_by_timeline(cascades:Dict[int,list], user_texts_mp:Dict[int,dict], g:igraph.Graph)->Dict[int,list]:
    ...
    # save_pickle(tids_aggby_timeline_mp, "/remote-home/share/dmb_nas/ppx/HeterGAT/basic/tids_aggby_timeline_mp.pkl")
    # save_pickle(tid2text_mp, "/remote-home/share/dmb_nas/ppx/HeterGAT/basic/tid2text_mp.pkl")
    # save_pickle(tid2caselem_mp, "/remote-home/share/dmb_nas/ppx/HeterGAT/basic/tid2caselem_mp.pkl")
...
def build_topic_similarity_user_edges(ut_mp:dict):
    ...
    docs = load_pickle(f"/remote-home/share/dmb_nas/ppx/HeterGAT/tweet-embedding/bertopic/raw_texts_aggby_user_filter_lt2words_process_remove_hyphen.pkl")
    docs = flattern(docs)
    topic_labels = load_pickle(f"/remote-home/share/dmb_nas/ppx/HeterGAT/tweet-embedding/bertopic/topic_label_{suffix}.pkl")
    topic_probs  = load_pickle(f"/remote-home/share/dmb_nas/ppx/HeterGAT/tweet-embedding/bertopic/topic_prob_{suffix}.pkl")
    topic_reprs  = load_pickle(f"/remote-home/share/dmb_nas/ppx/HeterGAT/tweet-embedding/bertopic/topic_representation_{suffix}.pkl")

    # ut_mp = load_pickle("/remote-home/share/dmb_nas/ppx/HeterGAT/basic/ut_mp_filter_lt2words_processedforbert_subg.pkl")
...
def build_heteredge_mats2(data_dict:dict, _u2idx:dict, window_size:int, n_component:int, dataset:str='Weibo-Aminer'):
    ...
    user_edges = load_pickle(f"/remote-home/share/dmb_nas/ppx/HeterGAT/{dataset}/edges.data")
    classid2simmat = merge_user_edges_with_topic_edges(user_edges, classid2simedges, user_size=len(_u2idx), add_self_loop=True)
    save_pickle(classid2simmat, f"/remote-home/share/dmb_nas/ppx/HeterGAT/{dataset}/topic_diffusion_graph_windowsize{window_size}.data")
    ...
    save_pickle(classid2simmat2, f"/remote-home/share/dmb_nas/ppx/HeterGAT/{dataset}/topic_diffusion_motif_graph_windowsize{window_size}.data")
