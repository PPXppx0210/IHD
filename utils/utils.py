...
config.read(os.path.join(PROJECT_ROOT, 'src/config.ini'))
DATA_ROOTPATH = config['DEFAULT']['DataRootPath']
if not os.path.isabs(DATA_ROOTPATH):
    DATA_ROOTPATH = os.path.normpath(os.path.join(PROJECT_ROOT, DATA_ROOTPATH))
...
def save_model(epoch, args, model, optimizer, filepath:str=None):
    ...
    save_dirpath = os.path.dirname(save_filepath)
    if save_dirpath:
