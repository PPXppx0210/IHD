...
from torch_geometric.nn import GCNConv, GATv2Conv, HypergraphConv, TransformerConv
    ...
    def forward(self, adj:torch.Tensor, seq_embs:torch.Tensor, time_decay_emb:torch.Tensor, pad_mask:torch.Tensor=None):
        ...
        for layer_i, gat_layer in enumerate(self.layer_stack,):
            ...
            if torch.isnan(emb).any():
                print(f"NaN detected in layer {layer_i}!")
                print("Input stats:", seq_embs.min(), seq_embs.max())
                print("Adj stats:", adj.min(), adj.max())
                raise ValueError("NaN in GAT layer output")
...
class BasicGATNetwork(nn.Module):
    ...
    def forward(self, cas_uids, cas_intervals, graph):
        ...
        seq_embs = F.dropout(seq_embs, self.dropout, training=self.training)
...
class SemanticSequenceNetwork(nn.Module):
    def __init__(self, user_size, semantic_feat_dim, hidden_dim, num_interval, dropout):
        super(SemanticSequenceNetwork, self).__init__()

        self.user_size = user_size
        self.dropout = dropout
        pos_emb_dim = 8
        self.feat_fc_layer = nn.Linear(semantic_feat_dim, hidden_dim)
        self.pos_emb = nn.Embedding(1000, pos_emb_dim)
        self.time_attention = TimeAttention_New(ninterval=num_interval, nfeat=hidden_dim + pos_emb_dim)
        self.decoder_attention = TransformerBlock(input_size=hidden_dim + pos_emb_dim, n_heads=8)
        self.query_fc = nn.Linear(hidden_dim + pos_emb_dim, hidden_dim)
        self.init_weights()

    def init_weights(self):
        init.xavier_normal_(self.feat_fc_layer.weight)
        init.xavier_normal_(self.query_fc.weight)
        init.xavier_normal_(self.pos_emb.weight)

    def forward(self, cas_uids: torch.Tensor, cas_intervals: torch.Tensor, semantic_feats: torch.Tensor):
        if semantic_feats is None:
            raise ValueError("SemanticSequenceNetwork requires semantic_feats.")

        cas_uids = cas_uids[:, :-1]
        cas_intervals = cas_intervals[:, :-1]
        mask = (cas_uids == PAD)

        user_semantic_embs = self.feat_fc_layer(semantic_feats.to(cas_uids.device))
        seq_embs = F.embedding(cas_uids, user_semantic_embs)
        seq_embs = F.dropout(seq_embs, self.dropout, training=self.training)

        batch_t = torch.arange(cas_uids.size(1)).expand(cas_uids.size()).to(cas_uids.device)
        pos_embs = F.dropout(self.pos_emb(batch_t), self.dropout, training=self.training)
        seq_embs = torch.cat([seq_embs, pos_embs], dim=-1)
        seq_embs = self.time_attention(cas_intervals, seq_embs, mask)
        seq_embs = self.decoder_attention(seq_embs, seq_embs, seq_embs, mask)

        query = self.query_fc(seq_embs)
        scores = torch.matmul(query, user_semantic_embs.transpose(0, 1))
        scores = scores + get_previous_user_mask(cas_uids, self.user_size)
        return scores


class SemanticGATNetwork(nn.Module):
    def __init__(
        self, user_size, semantic_feat_dim, n_feat, n_units, n_heads,
        num_interval, attn_dropout, dropout, use_gat=True,
    ):
        super(SemanticGATNetwork, self).__init__()

        self.user_size = user_size
        self.dropout = dropout
        pos_emb_dim = 8
        self.feat_fc_layer = nn.Linear(semantic_feat_dim, n_feat)
        if use_gat:
            self.graph_network = GATNetwork(n_feat, n_units, n_heads, attn_dropout, dropout)
        else:
            self.graph_network = GCNNetwork(n_feat, n_units, dropout)
        out_dim = n_units[-1] * n_heads[-1] if use_gat else n_units[-1]
        self.pos_emb = nn.Embedding(1000, pos_emb_dim)
        self.decoder_attention = TransformerBlock(input_size=out_dim + pos_emb_dim, n_heads=8)
        self.query_fc = nn.Linear(out_dim + pos_emb_dim, out_dim)
        self.init_weights()
    ...
    def forward(self, cas_uids: torch.Tensor, cas_intervals: torch.Tensor, graph, semantic_feats: torch.Tensor):
        if semantic_feats is None:
            raise ValueError("SemanticGATNetwork requires semantic_feats.")

        cas_uids = cas_uids[:, :-1]
        mask = (cas_uids == PAD)

        user_semantic_embs = self.feat_fc_layer(semantic_feats.to(cas_uids.device))
        graph_embs = self.graph_network(graph, user_semantic_embs)
        seq_embs = F.embedding(cas_uids, graph_embs)
        seq_embs = F.dropout(seq_embs, self.dropout, training=self.training)
        ...
        query = self.query_fc(seq_embs)
        scores = torch.matmul(query, graph_embs.transpose(0, 1))
        scores = scores + get_previous_user_mask(cas_uids, self.user_size)
        return scores
...
class GraphTransformerEncoder(nn.Module):
    ...
    def forward(self, emb, graph):
        edge_index = graph.edge_index.to(emb.device)
        h = self.conv(emb, edge_index)
        h = self.norm(emb + self.dropout(h))
        h2 = self.ffn(h)
        h = self.ffn_norm(h + self.dropout(h2))
        return h


class SequentialHypergraphEncoder(nn.Module):
    def __init__(self, hidden_dim, num_interval, dropout):
        super(SequentialHypergraphEncoder, self).__init__()
        self.num_interval = num_interval
        self.hyper_conv = HypergraphConv(hidden_dim, hidden_dim)
        self.period_emb = nn.Embedding(num_interval, hidden_dim)
        self.update_gate = nn.Linear(hidden_dim * 2, hidden_dim)
        self.period_score = nn.Linear(hidden_dim, 1)
        self.norm = nn.LayerNorm(hidden_dim)
        self.dropout = nn.Dropout(dropout)
        self.init_weights()

    def init_weights(self):
        init.xavier_normal_(self.period_emb.weight)
        init.xavier_normal_(self.update_gate.weight)
        init.xavier_normal_(self.period_score.weight)

    def forward(self, static_emb, hypergraphs):
        state = static_emb
        period_states = []

        for period_i in range(self.num_interval):
            edge_index = hypergraphs[period_i].to(static_emb.device)
            if edge_index.numel() > 0:
                period_state = self.hyper_conv(state, edge_index)
            else:
                period_state = state
            period_tag = self.period_emb.weight[period_i].unsqueeze(0)
            period_state = F.relu(period_state + period_tag)
            period_state = self.dropout(period_state)
            gate = torch.sigmoid(self.update_gate(torch.cat([state, period_state], dim=-1)))
            state = self.norm(gate * period_state + (1 - gate) * state)
            period_states.append(state.unsqueeze(0))

        stacked = torch.cat(period_states, dim=0)
        score = self.period_score(stacked).squeeze(-1)
        alpha = F.softmax(score, dim=0).unsqueeze(-1)
        return torch.sum(alpha * stacked, dim=0)


class GTHNNNetwork(nn.Module):
    def __init__(self, user_size, hidden_dim, num_interval, n_heads, dropout, attn_dropout):
        super(GTHNNNetwork, self).__init__()
        self.user_size = user_size
        self.hidden_dim = hidden_dim
        self.dropout = dropout
        pos_emb_dim = 8

        self.user_emb = nn.Embedding(user_size, hidden_dim, padding_idx=PAD)
        self.static_encoder = GraphTransformerEncoder(hidden_dim, n_heads, attn_dropout)
        self.dynamic_encoder = SequentialHypergraphEncoder(hidden_dim, num_interval, dropout)
        self.fusion_gate = nn.Linear(hidden_dim * 2, hidden_dim)
        self.pos_emb = nn.Embedding(1000, pos_emb_dim)
        self.time_attention = TimeAttention_New(ninterval=num_interval, nfeat=hidden_dim + pos_emb_dim)
        self.decoder_attention = TransformerBlock(input_size=hidden_dim + pos_emb_dim, n_heads=8)
        self.fc_network = nn.Linear(hidden_dim + pos_emb_dim, user_size)
        self.init_weights()

    def init_weights(self):
        init.xavier_normal_(self.user_emb.weight)
        init.xavier_normal_(self.fusion_gate.weight)
        init.xavier_normal_(self.pos_emb.weight)
        init.xavier_normal_(self.fc_network.weight)

    def forward(self, cas_uids, cas_intervals, graph, hypergraphs):
        cas_uids = cas_uids[:, :-1]
        cas_intervals = cas_intervals[:, :-1]
        mask = (cas_uids == PAD)

        base_emb = self.user_emb(torch.arange(self.user_size, device=cas_uids.device))
        static_emb = self.static_encoder(base_emb, graph)
        dynamic_emb = self.dynamic_encoder(static_emb, hypergraphs)
        gate = torch.sigmoid(self.fusion_gate(torch.cat([static_emb, dynamic_emb], dim=-1)))
        user_embs = gate * dynamic_emb + (1 - gate) * static_emb

        seq_embs = F.embedding(cas_uids, user_embs)
        seq_embs = F.dropout(seq_embs, self.dropout, training=self.training)
        batch_t = torch.arange(cas_uids.size(1), device=cas_uids.device).expand(cas_uids.size())
        pos_embs = F.dropout(self.pos_emb(batch_t), self.dropout, training=self.training)
        seq_embs = torch.cat([seq_embs, pos_embs], dim=-1)
        seq_embs = self.time_attention(cas_intervals, seq_embs, mask)
        seq_embs = self.decoder_attention(seq_embs, seq_embs, seq_embs, mask)
        ...
        output = self.fc_network(seq_embs)
        output = output + get_previous_user_mask(cas_uids, self.user_size)
        return output
...
class HeterEdgeGATNetwork(nn.Module):
    ...
    def select_topics(self, aware_seq_embs:torch.Tensor, cas_classids:torch.Tensor,):
        ...
        selected_aware_seq_embs = F.dropout(selected_aware_seq_embs, self.dropout, training=self.training)
    ...
    def forward(self, cas_uids:torch.Tensor, cas_intervals:torch.Tensor, cas_classids:torch.Tensor, hedge_graphs, cas_tss=None, feats:torch.Tensor=None, multi_deepwalk_feat:torch.Tensor=None):
        ...
        if not self.use_gat: # use gcn
            ...
            for heter_i, gat_network in enumerate(self.heter_gat_network):
                ...
                zero_row_indices = torch.where(graph_adj.sum(dim=1) == 0)[0]
        ...
        # K = aware_seq_embs.size(2)
        # user_seq_exp = user_seq_embs.unsqueeze(2).expand(-1, -1, K, -1)  # (bs, ml, K, D)
        # fusion_input = torch.cat([aware_seq_embs, user_seq_exp], dim=-1)  # (bs, ml, K, 2D)
        # # reshape for MLP
        # bs, ml, k, d2 = fusion_input.size()
        # fusion_input = fusion_input.view(-1, d2)  # (bs * ml * K, 2D)
        # weights = self.graph_mlp(fusion_input).view(bs * ml, K)  # (bs * ml, K)
        # weights = F.softmax(weights, dim=-1)
        # aware_seq_embs = aware_seq_embs.view(bs * ml, K, -1)  # (bs * ml, K, D)
        # weighted_sum = torch.bmm(weights.unsqueeze(1), aware_seq_embs).squeeze(1)  # (bs * ml, D)
        # aware_seq_embs = weighted_sum.view(bs, ml, -1)  # (bs, ml, D)

        bs, ml, K, D = aware_seq_embs.size()
        fusion_input = aware_seq_embs.view(-1, D)
        weights = self.graph_mlp(fusion_input).view(bs * ml, K)  # (bs * ml, K)
        weights = F.softmax(weights, dim=-1)
        aware_seq_embs = aware_seq_embs.view(bs * ml, K, D)  # (bs * ml, K, D)
        weighted_sum = torch.bmm(weights.unsqueeze(1), aware_seq_embs).squeeze(1)  # (bs * ml, D)
        aware_seq_embs = weighted_sum.view(bs, ml, D)  # (bs, ml, D)
        
        seq_embs = F.dropout(aware_seq_embs, self.dropout, training=self.training)
